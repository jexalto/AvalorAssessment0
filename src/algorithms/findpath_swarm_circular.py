...
class FindPathSwarm(FindPathGreedyTwoLayers):
    ...
    def _find_closest_circle(self) -> dict:
        ...
        dronegrid_properties_copy = copy.deepcopy(self.dronegrid_properties)

        for drone in dronegrid_properties_copy:
            radii = divide_grid_circular(
                nr_drones=self.nr_drones, grid=drone.grid)
            circular_grids = create_circular_grids(
                radii=radii, grid=drone.grid)

            distances_drones_circles.append(
                drone.distance_to_circle(radii=radii))
        ...
        for drone_index, drone in enumerate(self.dronegrid_properties):
            ...
            drone.circular_grid(
                grid=circular_grids[circle_drone_index[drone_index]])
    ...
    def _initialise_drones(self) -> None:
        ...
        for index, idronegrid in enumerate(self.dronegrid_properties):
            ...
            idronegrid.drone.add_to_sum(
                square_value=idronegrid.grid.gridvalues[current_drone_coords[1]][current_drone_coords[0]])
    ...
    def _swarm_policy(self, total_time) -> None:
        ...
        '''
        radii = divide_grid_circular(
            nr_drones=self.nr_drones, grid=self.dronegrid_properties[0].grid)
        ...
        for timestep in range(total_time):
            for drone_index, drone in enumerate(self.dronegrid_properties):
                ...
                drone_in_section, direction = drone.drone_direction(
                    radii=radii, radius_index=radius_index)

                if drone_in_section:
                    # perform standard two layer drone algo
                    pathvalues = drone.get_surrounding_values_circular()
                    self.dronegrid_properties[drone_index] = self._update_dronegridinfo(
                        pathvalues=pathvalues, drone=drone, timestep=timestep)
    ...
    def _sync_grids(self):
        '''
            ...
            TODO: O(N*N*X) This function is spectacularly inefficient but idh time to improve it rn
                    To improve this part introduces a shared grid_multiplier matrix. This requires rewriting all dataclasses though.
        '''
...
class FindPathSwarmGif(FindPathSwarm):
    def _swarm_policy(self, total_time) -> None:
        ...
        # these values are important for the post-processing
        dronegriddict['total_time'] = total_time
        dronegriddict['nr_drones'] = self.nr_drones
        dronegriddict['radii'] = radii

        # sync grids for first run
        self._sync_grids()

        with open(os.path.join(BASE_DIR, 'data', 'gifs', 'dronegrid_data',  'gridinfo.json'), 'w') as file:
            dronegriddict['grid_t' +
                          str(-1)] = self.dronegrid_properties[0].grid.grid_multiplier.flatten().tolist()
            for timestep in range(total_time):
                for drone_index, dronegrid in enumerate(self.dronegrid_properties):
                    radius_index = self.circle_drone_index[drone_index]

                    drone_in_section, direction = dronegrid.drone_direction(
                        radii=radii, radius_index=radius_index)
                    ...
                    if drone_in_section:
                        ...
                        self.dronegrid_properties[drone_index] = self._update_dronegridinfo(
                            pathvalues=pathvalues, drone=dronegrid, timestep=timestep)
                ...
                dronegriddict['grid_t'+str(timestep)
                              ] = dronegrid.grid.grid_multiplier.flatten().tolist()

            for drone_index, dronegrid in enumerate(self.dronegrid_properties):
                dronegriddict['drone_path' +
                              dronegrid.drone.name] = dronegrid.drone.path
                dronegriddict['drone_path_values' +
                              dronegrid.drone.name] = dronegrid.drone.total_path_value
