...
class FindPathGreedyTwoLayers:
    ...
    def _initialise(self, dronegrid_properties: list[DroneGridInfo]) -> None:
        ...
        for index, idronegrid in enumerate(dronegrid_properties):
            # copy variable
            idronegrid = copy.deepcopy(idronegrid)
            # reset grid to account for previous runs with same object
            ...
            starting_x, starting_y = idronegrid.drone.starting_point
            # TODO: integrate this in the drone-grid initialisation
            # idronegrid.grid.drone_moved_to_square(coords=[starting_x, starting_y], time=-1)
            idronegrid.grid.grid_multiplier[starting_y][starting_x] = 0
            idronegrid.grid.gridvalues[starting_y][starting_x] = 0

            self.dronegrid_properties[index] = idronegrid
    ...
    def _max_index_finder(self, surrounding_values: list[int], current_grid: GridInfo, current_coords: list[int]) -> int:
        ...
        max_values_indices = np.where(
            surrounding_values == surrounding_values[max_indices])[0]
    ...
    def _process_paths(self, total_time: int) -> list[DroneGridInfo]:
        ...
        for index, idronegrid in enumerate(self.dronegrid_properties):
            ...
            idronegrid.drone.add_to_sum(
                square_value=idronegrid.grid.gridvalues[new_drone_coords[1]][new_drone_coords[0]])

            # === Update grid ===
            idronegrid.grid.drone_moved_to_square(
                coords=new_drone_coords, time=1)
        ...
        for timestep in range(2, total_time+1):
            for drone_index, idronegrid in enumerate(self.dronegrid_properties):
                self.dronegrid_properties[drone_index] = self._move_drone(
                    drone=idronegrid, timestep=timestep)
    ...
    def _update_dronegridinfo(self, pathvalues: np.array, drone: DroneGridInfo, timestep: int):
        ...
        assert any(new_drone_coords) >= 0 and any(
            new_drone_coords) < self.dronegrid_properties[0].grid.size[0]

        # === Update drone path ===
        drone.drone.move_drone(coords_new=new_drone_coords)
        drone.drone.add_to_sum(
            square_value=drone.grid.gridvalues[new_drone_coords[1]][new_drone_coords[0]])

        # === Update grid ===
        drone.grid.drone_moved_to_square(
            coords=new_drone_coords, time=timestep)
